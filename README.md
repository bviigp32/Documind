...
* **기간:** 2026.03.09 ~ (진행 중)
* **역할:** AI 백엔드 엔지니어링 (데이터 전처리, 임베딩 파이프라인, 벡터 DB 구축, LLM 연동, API 서빙, 도커 컨테이너화)
...
5. **Containerization (배포 환경):** `Docker`를 활용해 애플리케이션과 벡터 DB를 독립적인 컨테이너 환경으로 패키징하여 배포 안정성 확보.
...
# Step 1: 문서 임베딩 및 벡터 DB 구축 (최초 1회 필수)
python src/ingest.py

# Step 2: FastAPI 서버 실행
uvicorn src.main:app --reload

# 실행 후 브라우저에서 http://localhost:8000/docs 로 접속하여 API 테스트 진행
...
# Step 2: 컨테이너 실행
docker run -d -p 8000:8000 --env-file .env --name documind-container documind-api
