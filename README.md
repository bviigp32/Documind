...
## 프로젝트 개요 (Overview)
* **목표:** 사내 문서(PDF 등)를 기반으로 환각 현상(Hallucination) 없이 정확한 정보를 제공하는 RAG 기반 질의응답 시스템 구축.
* **기간:** 2026.03.09 ~ (진행 중)
* **역할:** AI 백엔드 엔지니어링 (데이터 전처리, 임베딩 파이프라인, 벡터 DB 구축, 비동기 분산 처리, 대화형 메모리 관리, 프론트엔드 연동 및 트러블슈팅)
...
3. **Task Synchronization (상태 동기화):** 프론트엔드의 폴링(Polling) 로직을 통해 백그라운드 작업 상태를 주기적으로 확인하여 DB Lock 현상을 방지하고 안정적인 UX 제공.
4. **Embedding & Vector Storage (벡터 변환 및 저장):** `HuggingFace` 오픈소스 모델을 활용해 텍스트를 고차원 벡터로 변환(Embedding)하고, `ChromaDB`에 적재.
5. **Conversational Memory (대화 기록 관리):** `PostgreSQL` 데이터베이스를 연동하여 사용자의 세션별 질문과 답변 기록을 영구 저장하고, 연속적인 대화의 문맥(Context)을 유지.
6. **Retrieval-Augmented Generation (검색 증강 생성):** 과거 대화 기록과 벡터 검색 결과를 프롬프트에 주입하여 **Google Gemini 2.5 Flash** 모델이 답변을 생성.
7. **Client Interface (사용자 인터페이스):** `Streamlit`을 활용하여 파일 업로드 및 실시간 채팅이 가능한 직관적인 웹 화면 제공.

## 개발 로그 (Development Log)
* **Phase 1~4 (기반 세팅 및 RAG 파이프라인 구축):** FastAPI 환경 세팅, 문서 청킹, HuggingFace 임베딩, ChromaDB 적재 및 Google Gemini 2.5 Flash 연동. (Day 1~4)
...
* **Phase 8 (대화형 메모리 구축):** `PostgreSQL`을 연동하여 세션별 대화 기록을 저장하고, 문맥을 기억하는 지능형 대화형 RAG 시스템 완성. (Day 8)
* **Phase 9 (프론트엔드 구축):** `Streamlit`을 활용하여 사용자 친화적인 대화형 웹 UI 구현. (Day 9)
* **Phase 10 (트러블슈팅 및 아키텍처 고도화):** Celery 워커와 API 서버 간의 작업 시차로 인해 발생하는 DB Lock(SQLite 한계) 문제를 분석하고, 상태 조회 API(`/task/{id}`) 및 프론트엔드 폴링(Polling) 로직을 도입하여 시스템 안정성 확보. (Day 10)
